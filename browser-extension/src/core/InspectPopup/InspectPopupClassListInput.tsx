...
export default function InspectPopupClassListInput({ onChangeClasses }: { onChangeClasses: (classNames: string) => void }) {
  ...
  const updateCSS = (debouncedInput: string) => {
    let element = document.querySelector('[data-inspect-popup-class-style-list]') as HTMLElement;
  ...
  const copy = () => {
    navigator.clipboard.writeText(debouncedInput);
  }
  ...
  return (
    <div className={"flex gap-1 py-1"}>
      <input
        ...
        className={"border rounded-[2px] border-solid border-gray-400 placeholder:font-normal placeholder:text-gray-400 px-1"}
