...
const Options = () => {
  ...
  const { data: editorsData } = useQuery<Editor[]>({
    queryKey: ["editors"],
    queryFn: async () => {
  ...
  const editorMutation = useMutation({
    mutationFn: async (editor: Editor) => {
      const currentEditors =
        (await chrome.storage.local.get(["editors"])).editors || defaultEditors;
      const updatedEditors = currentEditors.map((e: Editor) =>
  ...
  const addEditorMutation = useMutation({
    mutationFn: async (newEditor: { name: string; path?: string }) => {
      const currentEditors =
        (await chrome.storage.local.get(["editors"])).editors || defaultEditors;
      const updatedEditors = [
        ...currentEditors,
        { ...newEditor, id: newEditor.name.toLowerCase() },
      ];
      await chrome.storage.local.set({ editors: updatedEditors });
      return updatedEditors;
    },
  ...
  const { data: extensionDevelopmentPathData } = useQuery<string | undefined>({
  ...
  const { data: showArrowControls } = useQuery<boolean>({
    ...
    <div className="quick-edits flex flex-col pb-4 dark:bg-[#292929] bg-white font-sans dark:text-white text-black min-w-[500px] min-h-full">
        ...
        <div className="flex gap-2 items-center">
          <input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
              arrowControlsMutation.mutate(e.target.checked)
            }
          />
          <label
            htmlFor="showArrowControls"
      ...
      <SectionBody className={"!gap-2"}>
        {data?.map((mapping) => (
          <div
            key={mapping.pattern}
            className="flex items-center justify-between p-3 border border-gray-200 dark:border-[#3c4043] hover:bg-gray-50 dark:hover:bg-[#2c2e31] transition-colors rounded-lg"
          >
            <div className="flex flex-col gap-2">
              <div className="font-mono font-medium text-gray-900 dark:text-[#e8eaed]">
                {mapping.pattern}
              </div>
              <div className="text-gray-600 dark:text-[#9aa0a6] font-mono">
                {mapping.searchFolder}
              </div>
            </div>
            <div className="flex gap-2">
              <button
                type="button"
                className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-[#383a3d] text-gray-600 dark:text-[#9aa0a6] hover:text-gray-900 dark:hover:text-[#e8eaed] transition-all"
      ...
      <SectionBody className="gap-0">
        ...
        {editorsData?.map((editor) => (
          <div
            key={editor.id}
            className="flex items-center justify-between p-3 border border-[#3c4043] hover:bg-[#2c2e31] transition-colors rounded-lg"
          >
            <div className="flex flex-col gap-1">
              <div className="font-mono font-medium text-[#e8eaed]">
                {editor.name}
              </div>
              <div className="text-[#9aa0a6] font-mono">
                {editor.path || "Default path"}
              </div>
            </div>
            <div className="flex gap-2">
              <button
